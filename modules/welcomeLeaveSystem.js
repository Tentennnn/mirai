		...
		try {
			...
			Schema.findOne(
				...
				async (e, data) => {
					...
					var textString4 = `YOU ARE THE ${
						member.guild.memberCount
					}TH MEMBER OF OUR SERVER`;
					ctx.textAlign = 'center';
					...
					await channel
						.send({
							content: data.WelcomeMsg.replace(/<@>/g, `${member}`)
				...
				async (e, data) => {
					...
					await channel
						.send({
							content: `Look like **${
								member.user.tag
							}** just left the server <a:crying:910146626327285781>`,
							files: [attachment]
						})
